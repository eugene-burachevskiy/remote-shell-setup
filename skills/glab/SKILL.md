...
# Merge requests
glab mr create --push --title "Fix session timeout" --description "$(cat /tmp/desc.md)"
glab mr view <iid>
...
**Templates:** Use the applicable file in `.gitlab/merge_request_templates/`
or `.gitlab/issue_templates/` when one exists.

### Merge request writing standard

Before `glab mr create` or `glab mr update`, review the complete branch diff,
its commits, and related issue context.

- Follow an established repository title convention. Otherwise, write a short,
  sentence-case title in the imperative mood without a final period.
- Target 50 characters for a title. Never exceed 72 characters.
- State the problem, the change, the reason, verification, and important risks.
- Do not claim tests that did not run.
- Use active voice, present tense, common words, and one term for each concept.
- Give each sentence one idea. Keep sentences to 25 words or fewer.
- Put conditions before instructions. Use numbered lists for procedures.
...
These rules apply ASD-STE100 principles and the Google developer documentation
style. Project templates and required metadata take priority.
